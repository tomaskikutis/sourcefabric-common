...
export class GenericEvents<T extends {[key: string]: any}> {
    ...
    addListener<K extends keyof T>(
        ...
        window.addEventListener(`${this.prefix}__${eventName as string}`, handler as any, {once: options?.once});
    }

    removeListener<K extends keyof T>(eventName: K, handler: (event: CustomEvent<T[K]>) => void) {
        window.removeEventListener(`${this.prefix}__${eventName as string}`, handler as any);
    }

    dispatchEvent<K extends keyof T>(eventName: K, payload: T[K]): void {
        window.dispatchEvent(
            new CustomEvent(`${this.prefix}__${eventName as string}`, {
                detail: payload,
            }),
        );
    }
